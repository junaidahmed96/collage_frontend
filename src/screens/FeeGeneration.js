...
import { connect } from "react-redux";
 ...
 class FeeGeneration extends Component {
    constructor(props) {
        super(props);
        this.state = {
            month: 0,
    ...
    handleChange(event) {
        ...
        this.setState({ permonth:  parseInt(this.state.totalamount) /event.target.value ,month:event.target.value});
    ...
    handleRemaining(event) {
      ...
      async rollnoHandler (e)  {
        ...
        setTimeout(async() => {
            ...
            console.log(c);
            let res = await api.feegeneration(c)
            console.log(res.result);
            if (res.success=='true') {
                this.setState({className:res.result[0].className,totalamount:res.result[0].fee})
        ...
        return (
            ...
            <div className='admin-page add-new-student'>
                ...
                <div className='admin-heading'>
                    ...
                    <h2 className="hide-on-print">Fee Generation</h2>
                </div>
                <hr />
                <form onSubmit={this.myfunc} >
                    <div className="form-row  ">
                        ...
                        <div class="form-group col-md-3 onprint">
                            <label for="paidfee">Per Month Fee</label>
                            <input type="text" disabled class="form-control form-control-sm" id="paidfee" placeholder="Paid Fee" value={this.state.permonth} onChange={(e)=>this.setState({paidfee:e.target.value})}></input>
                        </div>
                        <div class="form-group col-md-3 onprint">
                            <label for="fathername">Class Name</label>
                            <input type="text" disabled class="form-control form-control-sm" id="fathername" placeholder="" value={this.state.className} onChange={(e)=>this.setState({remainingfee:this.state.totalamount - this.state.Fee})} ></input>
                        </div>
                        <div class="form-group col-md-3 onprint">
...
export default connect(mapState, mapDispatch)(FeeGeneration);